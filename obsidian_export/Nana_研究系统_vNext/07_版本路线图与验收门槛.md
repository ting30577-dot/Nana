...
> `release_baseline_frozen=false`。产品负责人已为缺失的观察明细批准 D3 一次性
> 证据例外；当前唯一剩余门禁是把该决定与 manifest、验证结果和 commit 重新冻结。
...
`acceptance_complete_baseline_pending`、`d3_complete=true`。当前唯一未关闭门禁是
重新生成 manifest、绑定结果 commit、重跑冻结验证并置
`release_baseline_frozen=true`；完成前不得开始 Tauri 建设。
