...
> `release_baseline_frozen=true`。产品负责人已为缺失的观察明细批准 D3 一次性
> 证据例外；该决定、验证结果和最终 manifest 已绑定到结果 commit 并冻结。
...
> [!note] 2026-08-17 D3 一次性证据例外
> 目标用户确认 30 分钟观察已经结束，但原始观察表、澄清次数和 10 个状态问题答案
> 未保存。产品负责人随后明确指示将本次 D3 观察门登记完成。当前证据因此记录为
> `ACCEPT_BY_PRODUCT_OWNER_EVIDENCE_EXCEPTION`，而不是倒填为原量化规则通过；它不
> 声称用户满意，也不修改后续版本继续执行本节原始规则的要求。
...
此处曾记录 D3 的中间重冻结快照；最终冻结数字已经更新为 Python 446/446、Vitest 67/67、10/10 连续旅程和发布矩阵 27/27。最终 D3 权威为 `docs/CURRENT_D3_AUTHORITY.md`，后续阶段见 `docs/ACTIVE_STATE.json`。

# 2026-08-17 D3 验收完成、发布基线已冻结

登记观察决定后全量验证为 Python 446 passed / 0 skipped；原技术切片还包括 Vitest 67/67、projection
self-test、TypeScript、production build、10/10 连续完整旅程及 27/27 浏览器
release matrix，Playwright retries=0。运行时数据已迁出源码树，安装目录通过
212 项输出清单泄露门禁。30 分钟观察已按产品负责人一次性证据例外登记，D3 为
`acceptance_complete_release_baseline_frozen`、`d3_complete=true`、
`release_baseline_frozen=true`。最终 manifest 已重新生成，冻结验证已重跑，并绑定
观察结果 commit `278f3a4186d7d0f85f6caf715c2882d63c589fc6`。
