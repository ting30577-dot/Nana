...
当前契约已包含浏览器安全的 `PauseRun` / `ResumeRun`，以及 `StartRun.retry_of_run_id`；失败源校验、`runs.retry_of_run_id` 与 `run_retry_of_run` Relation 在同一事务落盘。D3 冻结实现权威为 `docs/CURRENT_D3_AUTHORITY.md`，当前阶段见 `docs/ACTIVE_STATE.json`。
