...
当前代码来自早期算法学习原型。它可以运行，但目前只是迁移工作区，不构成 `v0.2.0-alpha`。在旧入口退出、核心对象落地以前，不对外宣称已经进入 alpha。
...
- 当前工作区使用 `v0.2.0-dev`，只表示处于历史标签之后的未发布迁移态，不继承旧 `v0.2` 方案。
...
| 版本 | 状态 | 边界 |
| --- | --- | --- |
| `v0.2.0-dev` | 当前迁移态 | 允许遗留代码暂时存在；不发布、不把旧页面计入新产品范围。 |
| `v0.2.0-alpha` | 新骨架成立 | 在导出/迁移已有刷题数据后，刷题账本与旧算法工作台退出运行入口；建立 Research Thread、Source、Claim、Evidence、Method、Experiment、Insight 的最小数据与界面骨架；一个真实算法问题完成无 AI 的“问题 → 方法 → 实现 → 测试/反例 → 判断”闭环。 |
